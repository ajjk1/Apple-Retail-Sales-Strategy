...
# 안전재고 대시보드 로직은 전부 model-server/05.Inventory Optimization/Inventory Optimization.py 에 있음.
# main.py 는 해당 모듈 로드 후 위 get_* 변수에 바인딩하여 라우트에서 호출만 함 (README §8.1 역할 분리).
...
# Inventory Optimization 모듈: 안전재고 대시보드 로직 전부 여기서 로드 (README: 역할 분리 — main.py는 라우트·호출만)
...
if _inv_opt_file.exists() and load_sales_dataframe is not None:
    try:
        ...
        get_safety_stock_summary = getattr(_inv_opt_module, "get_safety_stock_summary", None)
        get_kpi_summary = getattr(_inv_opt_module, "get_kpi_summary", None)
        get_inventory_list = getattr(_inv_opt_module, "get_inventory_list", None)
        get_demand_forecast_chart_data = getattr(_inv_opt_module, "get_demand_forecast_chart_data", None)
        get_sales_by_store_six_month = getattr(_inv_opt_module, "get_sales_by_store_six_month", None)
        get_sales_by_product = getattr(_inv_opt_module, "get_sales_by_product", None)
        get_inventory_critical_alerts = getattr(_inv_opt_module, "get_inventory_critical_alerts", None)
        get_inventory_health_for_recommendation = getattr(_inv_opt_module, "get_inventory_health_for_recommendation", None)
        ...
        print(f"[Apple Retail API] Inventory Optimization 로드 실패: {e}")
